...
#    Then "Sign Up form" "#commonLogin > div > form.form-holder.switchTab.commonForm.active > div" is displayed
  ...
  Background:
    Given User prints comment "User navigates to ThinkMobiles page"
    When User navigates to the "ThinkMobiles login" page with url "https://thinkmobiles.com/login/"
    Then Page title is equal to "ThinkMobiles"

  Scenario: 6:
    When User waits 4000 seconds
    And User clicks "Email field" ".active [name='email']"
    And User waits 4000 seconds
    And User enters "in email field" ".active [name='email']" text "ksenia@rfpmaker.com"
    And User enters "in Password field" ".active [name='password']" text "k1991508v2009"
    And User clicks "Log in button" "#urlTabsWrap > form.form-holder.urlTab.regForm.active > div > div.btn-group.center > button"
    Then User navigates to the "Account" page with url "https://thinkmobiles.com/profile/account"